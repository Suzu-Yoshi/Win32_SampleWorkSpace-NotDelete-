...
/*
 ...
 ４．関数を追加する
	...
	void MY_DRAW_POLYGON(HDC hdc)
	{
		...
		{
			//中心X座標+大きさ*弧度(360度/図形の角の数*右回りで何番目の角+90度左回転)
			rect_po[i].x = chusin.x + scale * cos(2 * MY_PI / kaku_cnt * i + left90);
			//中心Y座標+大きさ*弧度(360度/図形の角の数*右回りで何番目の角+90度左回転)
...
５．【WM_PAINT】メッセージ内の修正
	１．SaveDC(hdc);から、RestoreDC(hdc, -1);までの関数で、
		MY_DRAW(hdc);以外の関数を、全てコメントアウトする
...
６．プログラムが正常に実行されたら・・・
	・課題１：パックマンを描画してみよう
	  参考：MY_DRAW_GRAPH 関数
	・課題２：六角形を描画してみよう
	  参考：MY_DRAW_POLYGON 関数
	・課題３：モンスターボールを描画してみよう
	  参考：MY_DRAW_GRAPH 関数
