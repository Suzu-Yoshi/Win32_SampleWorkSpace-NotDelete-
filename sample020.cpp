...
1-2.学生共有サーバ\担当\41.G1_Win32APIの中にある
	MY_BITMAP というフォルダを コピーする

1-3.自分のプロジェクトが保存されているフォルダに
	MY_BITMAP というフォルダを 貼り付ける
...
◎マクロ定義に追加する
	...
	//ドラゴンのビットマップ
	#define BMP_DRA_PASS	TEXT(".\\MY_BITMAP\\mon23\\mon_255.bmp")	//ドラゴンのビットマップの場所
...
◎プロトタイプ宣言を追加する
	VOID MY_DRAW_POLYPOLYGON(HDC);の下に追加する
...
◎グローバル変数に追加する
	int win_mode = (int)WIN_RESZ_OK;の下に追加する
...
◎WinMainに追加
	MY_SetClientSize(hwnd);に追加する

	//ダブルバッファリングの設定をする
	MY_SetDoubleBufferring(hwnd);

◎【WM_CREATE】メッセージ内の修正
	if (AddFontResourceEx(FNT_TANU_PASS, FR_PRIVATE, NULL) == 0)の分岐処理の後に追加
...
◎ビットマップを描画する関数を追加する
	VOID MY_DRAW_BITMAP(HDC hdc)
	{
		...
		TransparentBlt(
			...
			RGB(107, 140, 148));//透過する背景色(この色を転送しない)
		...
		//背景とマスクをANDで転送→マスクの黒い部分(0)は無視される
		BitBlt(
			hdc,						//描画するデバイスコンテキスト
...
◎【WM_PAINT】メッセージ内の修正
    ...
    ☆「描画」を行う関数をすべてコメントアウトする
	
	MY_DRAW_POLYPOLYGON(hdc);の後に追加
...
◎【WM_DESTROY】メッセージ内の修正
	RemoveFontResourceEx(FNT_TANU_PASS, FR_PRIVATE, NULL);のあとに追加する
